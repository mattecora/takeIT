<?php
  ...
  if (empty($_POST["user"]) || empty($_POST["pwd"]) || empty($_POST["mail"]) || empty($_POST["name"]) || empty($_POST["surname"]) || empty($_POST["age"]) || empty($_POST["university"]) || empty($_POST["sector"])) {
	...
	$db = db_connect();
  $user = explode("@", $_POST["mail"])[0];
	$query = $db->query("INSERT INTO user VALUES ('$_POST[user]', '$_POST[mail]', '$_POST[name]', '$_POST[surname]', '$_POST[password]', NULL, $_POST[age], '$_POST[university]', '$_POST[sector]', $_POST[motivation])");
	$db->close();
?>
